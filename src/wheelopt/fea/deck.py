...
class DeckBundle:
    ...
    #: Where the reference node sits, metres — the indenter's reference point for a contact
    #: case, the tread centroid for a tip case. Carried out because the *tip* cases leave one
    #: in-plane DOF free, and reading that free displacement as "inward" or "outward" needs to
    #: know which way the tread was pointing to begin with.
...
def build_deck(
    ...
    * **Prescribed tip.** No indenter, no contact pair, no friction. The tread node set is
    ...
    #
    # The element size is FLOORED, and that floor is the whole of TODO #12's second half.
    # Unfloored, `E / element_size` grows without bound as the mesh refines, and it is the
    # absolute penalty rather than the factor that decides whether a frictional run converges:
    # holding the factor at 5 while refining, 4 mm converges and 2.5 and 1.5 mm do not, but
    # holding the penalty at the 4 mm value all three do, and the two finest agree to 0.02%.
    # See `SolverSpec.contact_length_floor_m`.
    contact_length_m = max(mesh_size(mesh), solver.contact_length_floor_m, 1e-6)
    contact_k = solver.contact_stiffness_factor * hyper.initial_youngs_pa / contact_length_m
    ...
    return DeckBundle(
        ...
        ref_point_m=np.asarray(anchor, dtype=np.float64),
